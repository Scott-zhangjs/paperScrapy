    ...
    def __getConn():
        ...
        if MysqlPool.__pool is None:
            __pool = PooledDB(creator=MySQLdb, mincached=1, maxcached=20,
                              ...
                              db=db, use_unicode=False, charset='utf8', cursorclass=DictCursor)
...
if __name__ == '__main__':
    ...
    sql_select = "SELECT CCF_id, CCF_name, CCF_abbreviation, CCF_type " \
                 "FROM ccf WHERE CCF_id<10000000 AND CCF_dblpname = %s"
    ...
    result = mysql.getAll(sql_select,("NOT IN DBLP", ))
    if result:
        print "get all"
        for row in result:
            print "%s\t%s" % (row["CCF_id"], row["CCF_name"])


    # mysql2 = MysqlPool()
    # result = mysql2.getOne(sql_select)
    # print "%s\t%s" % (result["CCF_id"], result["CCF_name"])
